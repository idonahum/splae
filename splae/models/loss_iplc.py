...
def dice_loss(predict,target, ent_dice_weight_map=None):
    target = target.float()
    smooth = 1e-4
    if ent_dice_weight_map is None:
        intersect = torch.sum(predict*target)
        dice = (2 * intersect + smooth)/(torch.sum(target)+torch.sum(predict*predict)+smooth)
    else:
...
class DiceLoss(nn.Module):
    ...
    def forward(self,inputs, target, ent_dice_weight_map=None, one_hot=True):
        x_shape = list(target.shape)
        if(len(x_shape) == 5):
            [N, C, D, H, W] = x_shape
            new_shape = [N*D, C, H, W]
